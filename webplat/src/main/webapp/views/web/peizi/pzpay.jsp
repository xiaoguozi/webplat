         ...
         <input type="hidden" name="dataId" value="${peizi.dataId}"/>
            <div class="tjs_1108px center">
                <div class="pz_produce">
                    <img src="assets/img/peizi/peizisq.png" width="34" alt=""><b><c:if test="${peiziType=='10'}">免费配</c:if>
                        	<c:if test="${peiziType=='20'}">天天配</c:if>
                        	<c:if test="${peiziType=='30'}">月月配</c:if>
                        	<c:if test="${peiziType=='40'}">低息1配1</c:if>
                        	<c:if test="${peiziType=='50'}">免息配</c:if>配资申请</b>
                </div>
                <div class="bgcolor">
                	<form name="payForm" class="hform" id="payForm" action=""  method="post">
                    ...
                    <div style="background: #F9F9F9; border: 2px solid #F0F0F0; width: 985px; height: 100px; margin-top:30px; margin-left: 55px;">
                        ...
                        <p class="pt10" style="text-align: center;"><span style="color: #C5C6C5;">购买产品：</span><span>
                        	<c:if test="${peiziType=='10'}">免费配</c:if>
                        	<c:if test="${peiziType=='20'}">天天配</c:if>
                        	<c:if test="${peiziType=='30'}">月月配</c:if>
                        	<c:if test="${peiziType=='40'}">低息1配1</c:if>
